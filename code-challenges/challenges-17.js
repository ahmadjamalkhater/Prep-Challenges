...
const recursionPattern = (int1, int2) => {
    ...
    var pattern1 = recursionPattern(int1 - int2, int2);
    return [int1].concat(pattern1, pattern1.length === 0 ? [] : [int1]);
}
// -------------------------------------------------------------------------------------------------------
...
const filterLinks = (str) => {
    // write your code here
    const regex = /href="https?:\/\/([^\/\s]+\.(com|org|net))"/i;
...
const isPalindrome = (str) => {
    // write your code here
    str = str.toLowerCase();
    let plaindRomeString = "";
    for (let i = 0; i < str.length; i++) {
        if (str[i] !== " " && str[i] !== "," && str[i] !== ":") {
            plaindRomeString += str[i];
        }
    }
    let reverseString = "";
    for (let i = plaindRomeString.length - 1; i >= 0; i--) {
        reverseString += plaindRomeString[i];
    }
    if (reverseString === plaindRomeString) {
        return true;
    }
    else {
        return false;
    ...
    if (str.length !== arr.length) {
      ...
      for (let i = 0; i < str.length; i++) {
        ...
        if (map.has(char)) {
          ...
          if (Array.from(map.values()).includes(element)) {
            return false;
          }
